...
SOURCE_NAME="${3:-trivy}"
IN_JSON="${REPORT_DIR}/trivy-image-report-v1.0.21.json"
OUT_HTML="${REPORT_DIR}/trivy-report.html"
...
"${PYTHON_BIN}" -m sec_report_kit render trivy --input "${IN_JSON}" --output "${OUT_HTML}" --source "${SOURCE_NAME}" --target "${TARGET_NAME}"
