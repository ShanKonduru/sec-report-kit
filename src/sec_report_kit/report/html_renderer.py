  ...
  for file_name in file_names:
    for base in base_dirs:
      ...
      if not (candidate.exists() and candidate.is_file()):
        continue
      if modified_since is not None or modified_until is not None:
        mtime = dt.datetime.fromtimestamp(
          candidate.stat().st_mtime,
          tz=(modified_since or modified_until).tzinfo,  # type: ignore[union-attr]
        )
        if modified_since is not None and mtime < modified_since:
          continue
        if modified_until is not None and mtime > modified_until:
          continue
      return candidate
  return None
...
def render_consolidated_dashboard_report(
  ...
  normalized_included_sources = {
    _normalize_tool_key(source)
    for source in (included_sources or set())
  }
  filtered_mode = modified_since is not None or modified_until is not None
  ...
  for spec in tool_specs:
    matched_file = _find_first_existing(
      base_dirs,
      spec["report_files"],
    ...
    if filtered_mode:
      # In filtered runs, only show embedded tool HTML if that tool had at least one
      # source file included in the filtered candidate set.
      available = available and spec["key"] in normalized_included_sources
    tool_rows.append(
      {
        "key": spec["key"],
        "name": spec["name"],
...
<main class="page">
  ...
  <div class="hero-meta">Generated: {_esc(generated_at)} | Reports available: {reports_available}/{reports_total} | Target: {_esc(target_ref)}{(' | Since: ' + _esc(modified_since.strftime('%Y-%m-%d %H:%M'))) if modified_since else ''}{(' | Until: ' + _esc(modified_until.strftime('%Y-%m-%d %H:%M'))) if modified_until else ''}</div>
  <div class="summary">
  ...
  <footer>
  <p>This report is generated by <a href="https://github.com/ShanKonduru/sec-report-kit" target="_blank" rel="noopener noreferrer">sec-report-kit utility</a> &nbsp;|&nbsp; Connect with the developer: <a href="https://www.linkedin.com/in/shankonduru/" target="_blank" rel="noopener noreferrer">Shan Konduru</a></p>
  </footer>
</main>
</body>
</html>
...
<body>
  ...
  <footer class="report-footer">
    This report is generated by <a href="https://github.com/ShanKonduru/sec-report-kit" target="_blank" rel="noopener noreferrer">sec-report-kit utility</a> &nbsp;|&nbsp; Connect with the developer: <a href="https://www.linkedin.com/in/shankonduru/" target="_blank" rel="noopener noreferrer">Shan Konduru</a>
  </footer>
</body>
</html>
"""
